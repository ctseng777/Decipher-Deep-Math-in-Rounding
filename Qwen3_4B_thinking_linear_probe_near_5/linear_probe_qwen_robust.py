...
# 
# This single Colab cell trains **streaming linear probes** that read "near‑5" (within 1 of a multiple of 5)
...
MODEL_ID_1 = "Qwen/Qwen3-4B-Thinking-2507"                     # e.g., "Qwen/Qwen2.5-0.5B-Instruct"
...
# Probing target: near‑5 = distance to nearest multiple of 5 <= 1 (covers last digits {0,1,4,5,6,9})
NEAR_THRESHOLD = 1
...
def run_probes_for_model(model_id: str, tag: str, train_pack, eval_packs):
    ...
    for name, (texts_ev, y_ev, meta_ev, surf_ev) in eval_packs.items():
        ...
        store_logits = {L: {0:[], 1:[], 2:[]}}  # distance buckets
        # Stream eval
        ...
        for Xb_per_layer, s in stream_hidden_features(model, tok, texts_ev, meta_ev, [bestL], BATCH_SIZE, MAX_LEN, surf_ev):
            ...
            for i, idx in enumerate(range(s.start, s.stop)):
                d, rd, n = meta_ev[idx]
                d_bucket = min(d, 2)  # 0/1 as-is; 2+ collapse to 2
