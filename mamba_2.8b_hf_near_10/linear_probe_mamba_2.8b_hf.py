...
MODEL_ID_1 = "state-spaces/mamba-2.8b-hf"                   # Mamba 2.8B HF model
MODEL_ID_2 = None                        # Optional: comparison model

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
DTYPE  = torch.float16 if DEVICE == "cuda" else torch.float32

# Data sizes (same as other models for comparison)
TRAIN_N = 4000
VAL_N   = 1500

# Efficiency (optimized for Mamba2-2.7B)
BATCH_SIZE  = 8      # smaller batch for 2.7B model
MAX_LEN     = 96     # Mamba2 can handle longer sequences efficiently
...
def model_layer_meta(model) -> Tuple[int,int]:
    ...
    with torch.no_grad():
        ...
        try:
            ...
            # Use Mamba-2.8B default values
            L = 64 + 1  # 64 layers + embedding
            D = 2560   # d_model for 2.8B
            print(f"  Using Mamba-2.8B defaults: L={L}, D={D}")
...
def run_probes_for_model(model_id: str, tag: str, train_pack, eval_packs):
    ...
    print("Loading Mamba-2.8B-HF state space model...")
    try:
...
with open("mamba_2.8b_hf_probe_results_near_10.json", "w") as f:
    json.dump(all_results, f, indent=2)
print(f"\nResults saved to mamba_2.8b_hf_probe_results_near_10.json")

print("\n" + "=" * 60)
print("Mamba-2.8B-HF Linear Probe Experiment Complete")
print("=" * 60)
print("Notes:")
print("- Model: state-spaces/mamba-2.8b-hf (State Space Model)")
